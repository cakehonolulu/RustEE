...
impl ComputeRenderer {
    pub fn new() -> Self {
        ...
        let make_pipeline =
            |entry: &'static str, layout: &wgpu::PipelineLayout, label: &'static str| {
        ...
        let pipeline_triangle =
            make_pipeline("cs_draw_triangle", &draw_pll, "GS Triangle Pipeline");
    ...
    fn enqueue_draw(&mut self, uniforms: DrawUniforms, wg_x: u32, wg_y: u32, is_sprite: bool) {
    ...
    fn encode_draw_batch(&self, encoder: &mut wgpu::CommandEncoder, batch: &[PendingCmd]) {
        ...
        for cmd in batch {
            ...
            {
                let pipeline = if *is_sprite {
                    &self.pipeline_sprite
                } else {
                    &self.pipeline_triangle
                };
    ...
    fn encode_blit(&self, encoder: &mut wgpu::CommandEncoder, cmd: &PendingCmd) {
        ...
        if height == 1 || width * 2 >= src_width {
            let first_pixel =
                src_base as u64 + src_rect_y as u64 * src_width as u64 + src_rect_x as u64;
    ...
    fn encode_pending(&mut self, encoder: &mut wgpu::CommandEncoder) {
        ...
        if self.draw_ring_slot > self.draw_ring_uploaded {
            ...
            self.queue
                .write_buffer(&self.draw_ring_buf, lo as u64, &self.draw_staging[lo..hi]);
        }
        ...
        if self.blit_ring_slot > self.blit_ring_uploaded {
            ...
            self.queue
                .write_buffer(&self.blit_ring_buf, lo as u64, &self.blit_staging[lo..hi]);
        }
    ...
    fn collect_readback_internal(&mut self, vram: &mut [u8]) {
        let Some(rb) = self.pending_readback.take() else {
            return;
        };
...
impl GsRenderer for ComputeRenderer {
    ...
    fn draw_triangle(
        ...
        let area = (v2_raw.x - v0.x) * (v1_raw.y - v0.y) - (v2_raw.y - v0.y) * (v1_raw.x - v0.x);
        if area == 0.0 {
            return;
        }
    ...
    fn transfer_hwreg(
        ...
        if byte_addr + 8 <= self.hwreg_shadow.len() {
            self.hwreg_shadow[byte_addr..byte_addr + 8].copy_from_slice(&hwreg_data.to_le_bytes());
    ...
    fn collect_readback(&mut self, vram: &mut [u8], _byte_offset: usize, _byte_len: usize) {
        self.collect_readback_internal(vram);
    }
    ...
    fn collect_to_display(
        ...
        self.collect_to_display_internal(
            back_buffer,
            dbx,
            buffer_width_pixels,
            read_width,
