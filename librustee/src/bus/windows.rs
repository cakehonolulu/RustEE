...
use super::{BUS_PTR, Bus, HW_BASE, HW_LENGTH};
use crate::bus::backpatch::AccessKind::*;
use crate::bus::backpatch::AccessWidth::*;
use crate::bus::backpatch::{
    AccessInfo, AccessKind, AccessWidth, execute_stub, find_function_end,
    find_memory_access_register, io_read8_stub, io_read16_stub, io_read64_stub, io_read128_stub,
    io_write8_stub, io_write16_stub, io_write64_stub, io_write128_stub,
};
use capstone::arch::x86::X86Reg;
use capstone::arch::{BuildsCapstone, DetailsArchInsn, x86};
use capstone::{Capstone, RegId, arch};
...
unsafe fn capture_raw_backtrace(skip: u32, max_frames: u32) -> Vec<*mut c_void> {
    unsafe {
        ...
        let captured =
            RtlCaptureStackBackTrace(skip, max_frames, buffer.as_mut_ptr(), ptr::null_mut());
...
unsafe extern "system" fn veh_handler(info: *mut EXCEPTION_POINTERS) -> i32 {
    unsafe { generic_exception_handler::<CurrentArchHandler>(info) }
}
...
>(
    ...
    if record.ExceptionCode != 0xC0000005u32 as i32 {
